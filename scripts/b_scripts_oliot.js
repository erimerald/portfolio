...
function haeAsunto() {
  ...
  // jos kohdenumeron kenttä ei ole tyhjä, tarkistaa löytyykö asunto-objektia asunnot-massiivista objektin kohdenumero-arvolla
    ...
    let haetunKohteenNumero = asunnot.find(
      (item) => item.kohdenumero == haeKohdenumero
    );
    // tulostaa löydetyn objektin kohdenumeron ja kutsuu objektin laskeNeliöhinta-metodia
    tulostus =
      "Haetun kohteen numero on " +
      haetunKohteenNumero.kohdenumero +
      ", ja sen neliöhinta on " +
      haetunKohteenNumero.laskeNelioHinta() +
      " € / m2";
